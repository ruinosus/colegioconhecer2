<%@ Control Language="C#" AutoEventWireup="true" CodeFile="ImagemSelecionar.ascx.cs"
    Inherits="ModuloImagem_ImagemSelecionar" %>
            ...
            <asp:GridView ID="GrdImagem" runat="server" AutoGenerateColumns="False" AllowPaging="True"
                ...
                <Columns>
                    <asp:TemplateField HeaderText="&amp;nbsp;">
                        <ItemTemplate>
                            <asp:RadioButton ID="idImagem" runat="server" value='<%# Eval("ID") %>' onclick='<%# GetOnSelectEvent(Eval("ID"))%>' />
                        </ItemTemplate>
                        <ItemStyle HorizontalAlign="Center" Width="2%" />
                    </asp:TemplateField>
                    <asp:TemplateField HeaderText="Título">
                        <ItemTemplate>
                            <asp:Label ID="lblTitulo" AssociatedControlID="idImagem" runat="server" Text='<%# Eval("Titulo") %>' />
                        </ItemTemplate>
